...
#import "CNKI_Z_GDataXMLNode.h"

@class NSArray, NSDictionary, NSError, NSString, NSURL;
@class CNKI_Z_GDataXMLElement, CNKI_Z_GDataXMLDocument;
...
// CNKIZGDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
    ...
    NSString *localName = [CNKI_Z_GDataXMLNode localNameForName:name];
...
@interface CNKI_Z_GDataXMLNode (PrivateMethods)
...
- (CNKI_Z_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (CNKI_Z_GDataXMLElement *)elementWithName:(NSString *)name {
...
+ (CNKI_Z_GDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
...
+ (CNKI_Z_GDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
    ...
    if (theXMLNode->type == XML_ELEMENT_NODE) {
        theClass = [CNKI_Z_GDataXMLElement class];
    } else {
        theClass = [CNKI_Z_GDataXMLNode class];
    }
...
#if DEBUG
    NSCAssert(chars != NULL, @"CNKIZGDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
    ...
    if (xmlNode_ != NULL) {
        ...
        while (currChild != NULL) {
            CNKI_Z_GDataXMLNode *node = [CNKI_Z_GDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (CNKI_Z_GDataXMLNode *)childAtIndex:(unsigned)index {
...
- (CNKI_Z_GDataXMLNode *)firstNodeForXPath:(NSString *)xpath error:(NSError **)error
{
...
- (CNKI_Z_GDataXMLNode *)firstNodeForXPath:(NSString *)xpath namespaces:(NSDictionary *)namespaces error:(NSError **)error
{
...
- (NSArray *)nodesForXPath:(NSString *)xpath
    ...
    // are created from CNKIZGDataXMLElement's initWithXMLString there may not be
    // a document. (We may later decide that we want to stuff the doc used
    // there into a CNKIZGDataXMLDocument and retain it, but we don't do that now.)
    //
        ...
        if (xpathCtx) {
            ...
            if (namespaces) {
                ...
                for (NSString *prefix in namespaces) {
                    ...
                    if (result != 0) {
#if DEBUG
                        NSCAssert1(result == 0, @"CNKIZGDataXMLNode XPath namespace %@ issue",
                                   prefix);
#endif
                    }
                }
                ...
                if (nsNodePtr != NULL) {
                    for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
                        ...
                        if (prefix == NULL) {
                            prefix = (xmlChar*) kCNKIZGDataXMLXPathDefaultNamespacePrefix;
                        }
                        
                        int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
                        if (result != 0) {
#if DEBUG
                            NSCAssert1(result == 0, @"CNKIZGDataXMLNode XPath namespace %s issue",
                                       prefix);
#endif
                        }
                    }
            ...
            if (xpathObj) {
                ...
                if (nodeSet) {
                    ...
                    for (int index = 0; index < nodeSet->nodeNr; index++) {
                        ...
                        CNKI_Z_GDataXMLNode *node = [CNKI_Z_GDataXMLNode nodeBorrowingXMLNode:currNode];
                        if (node) {
                            [array addObject:node];
                        }
                    }
...
- (BOOL)isEqual:(CNKI_Z_GDataXMLNode *)other {
    if (self == other) return YES;
    if (![other isKindOfClass:[CNKI_Z_GDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
    return (NSUInteger) (__bridge void *) [CNKI_Z_GDataXMLNode class];
}
...
@implementation CNKI_Z_GDataXMLElement
    ...
    if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
        ...
        while (currNS != NULL) {
            ...
            if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
                CNKI_Z_GDataXMLNode *node = [CNKI_Z_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)setNamespaces:(NSArray *)namespaces {
    ...
    if (xmlNode_ != NULL) {
        ...
        CNKI_Z_GDataXMLNode *namespaceNode;
...
- (void)addNamespace:(CNKI_Z_GDataXMLNode *)aNamespace {
...
- (void)addChild:(CNKI_Z_GDataXMLNode *)child {
...
- (void)removeChild:(CNKI_Z_GDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
    ...
    if (xmlNode_ != NULL) {
        ...
        for (CNKI_Z_GDataXMLNode *child in children) {
...
- (NSArray *)attributes {
    ...
    if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
        ...
        while (prop != NULL) {
            ...
            CNKI_Z_GDataXMLNode *node = [CNKI_Z_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
            if (array == nil) {
                array = [NSMutableArray arrayWithObject:node];
            } else {
                [array addObject:node];
...
- (void)addAttribute:(CNKI_Z_GDataXMLNode *)attribute {
...
- (CNKI_Z_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
    // search the cached attributes list for the CNKI_Z_GDataXMLNode with
    // the underlying xmlAttrPtr
    NSArray *attributes = [self attributes];
    
    for (CNKI_Z_GDataXMLNode *attr in attributes) {
...
- (CNKI_Z_GDataXMLNode *)attributeForName:(NSString *)name {
    ...
    if (xmlNode_ != NULL) {
        ...
        if (attrPtr) {
            CNKI_Z_GDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
            return attr;
        }
    }
    return nil;
...
- (CNKI_Z_GDataXMLNode *)attributeForLocalName:(NSString *)localName
...
@interface CNKI_Z_GDataXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
const char *CNKIZIANAEncodingCStringFromNSStringEncoding(NSStringEncoding encoding);
@end

@implementation CNKI_Z_GDataXMLDocument
...
- (id)initWithXMLString:(NSString *)str encoding:(NSStringEncoding)encoding error:(NSError **)error {
    ...
    CNKI_Z_GDataXMLDocument *doc = [self initWithData:data encoding:encoding error:error];
    return doc;
}
...
- (id)initWithHTMLString:(NSString *)str encoding:(NSStringEncoding)encoding error:(NSError **)error {
    ...
    NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
    CNKI_Z_GDataXMLDocument *doc = [self initWithHTMLData:data encoding:encoding error:error];
    return doc;
}
...
- (id)initWithData:(NSData *)data encoding:(NSStringEncoding)encoding error:(NSError **)error {
	...
	const char *xmlEncoding = CNKIZIANAEncodingCStringFromNSStringEncoding(encoding);
...
- (id)initWithRootElement:(CNKI_Z_GDataXMLElement *)element {
...
- (void)addStringsCacheToDoc {
    ...
    NSCAssert(xmlDoc_ != NULL && xmlDoc_->_private == NULL,
              @"CNKIZGDataXMLDocument cache creation problem");
#endif
...
- (CNKI_Z_GDataXMLElement *)rootElement {
    CNKI_Z_GDataXMLElement *element = nil;
    
    if (xmlDoc_ != NULL) {
        xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
        if (rootNode) {
            element = [CNKI_Z_GDataXMLElement nodeBorrowingXMLNode:rootNode];
        }
    }
    return element;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
    if (xmlDoc_ != NULL) {
        CNKI_Z_GDataXMLNode *docNode = [CNKI_Z_GDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
...
- (CNKI_Z_GDataXMLNode *)firstNodeForXPath:(NSString *)xpath namespaces:(NSDictionary *)namespaces error:(NSError *__autoreleasing *)error
{
	NSArray *nodes = [self nodesForXPath:xpath namespaces:namespaces error:error];
	if (!nodes.count) {
		return nil;
